define(function () {
	 ...
	 */
	{
		...
		parseToDom: function (s) {
			var res, parser;
			var parseError = new Error('XML is malformed and cannot be parsed.');
			if (typeof window.DOMParser != "undefined") {
				parser = new window.DOMParser();
				res = parser.parseFromString(s, "text/xml");
				var errorNs = parser.parseFromString('<', 'text/xml')
					.getElementsByTagName('parsererror')[0].namespaceURI;
				if (res.getElementsByTagNameNS(errorNs, 'parsererror').length) {
					throw parseError;
				}
				return res;
